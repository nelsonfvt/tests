...
% Se toman un voxel (aleatorio)
clc
i_vx = randperm(length(difs(:,1,1)),1);
vx1 = reshape(difs(i_vx(1),:,:), [30 30]);

%% Generar diferentes configuraciones (ex gradientes)
% sacar gradientes diferentes aleatorios (filas y columnas)
i_gr = sort(randperm(30,10));
D1 = vx1;
D1(i_gr,:) = [];
D1(:,i_gr) = [];

j_gr = sort(randperm(30,10));
D2 = vx1;
D2(j_gr,:) = [];
D2(:,j_gr) = [];
